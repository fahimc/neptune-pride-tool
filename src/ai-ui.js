var GameUI = {
    ...
    createContainer() {
        ...
        tabHolder.onclick = (event) => {
            ...
            document.querySelector('[data-target="finance"]').classList.remove('selected');
            ...
            switch (event.srcElement.getAttribute('data-target')) {
                ...
                case 'ai':
                    ...
                    document.querySelector('[data-target="ai"]').classList.add('selected');
                    break;
                case 'finance':
                    document.querySelector('.finance-info').classList.add('show')
                    document.querySelector('[data-target="finance"]').classList.remove('blue');
                    document.querySelector('[data-target="finance"]').classList.add('selected');
                    break;
            }
        }
    ...
    createPlayerInfo() {
        ...
        playerInfo.players.forEach(item => {
            ...
            const playerSection = this.createBoxSection(item.player, this.createTableFromObject(other).innerHTML);
    ...
    createAiInfo() {
        ...
        if ( this.previousRecommendations.length && JSON.stringify(this.previousRecommendations) == JSON.stringify(recommendations) && this.previousTranscript.length && JSON.stringify(this.previousTranscript) == JSON.stringify(transcript) && JSON.stringify(notifications) == JSON.stringify(this.previousNotificationInfo)) return;
        ...
        if (!currentNotificationSection) {
            const notificationSection = this.createBoxSection('INBOX', notificationHTML);
            notificationSection.classList.add('notif-section');
            section.appendChild(notificationSection)
        } else {
            currentNotificationSection.querySelector('.box-section').innerHTML = notificationHTML;
        }

        Array.prototype.forEach.call(section.querySelectorAll('.notification-close'), (item) => {
            item.onclick = () => {
        ...
        if (!currentTranscriptSection) {
            ...
            currentTranscriptSection.querySelector('.box-section').innerHTML = transHTML;
        }
    ...
    createFinanceInfo() {
        ...
        if (JSON.stringify(this.previousFinanceInfo) ==JSON.stringify( info)) return;
        ...
        section.appendChild(this.createBoxSection('Banking Income After Upgrade', this.createTableFromObject({
            currentPerProd: `$${info.banking.currentPerProd}`,
            currentLevel: `${info.banking.level}`,
            onUpgradePerProd: `$${info.banking.upgradePerProd}`,
        }).innerHTML))
        section.appendChild(this.createBoxSection('Manufactoring After Upgrade', this.createTableFromObject({
        ...
        section.appendChild(this.createBoxSection('Economy After Terraforming Upgrade', this.createTableFromObject({
        ...
        section.appendChild(this.createBoxSection('Industry After Terraforming Upgrade', this.createTableFromObject({
        ...
        section.appendChild(this.createBoxSection('Science After Terraforming Upgrade', this.createTableFromObject({
        ...
        section.appendChild(this.createBoxSection('Gate After Terraforming Upgrade', this.createTableFromObject({
