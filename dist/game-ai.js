...
var AI = {
  ...
  calcUCEUpgrade(total, r) {
    return Math.floor(
      (2.5 * NeptunesPride.gameConfig.developmentCostEconomy * (total + 1)) /
        (r / 100)
    );
  },
  calcUCE(t, r) {
    return Math.floor(
      (2.5 * NeptunesPride.gameConfig.developmentCostEconomy * (t.e + 1)) /
        ((r !== undefined ? r : t.r) / 100)
    );
  },
  calcUCI(t, r) {
    return Math.floor(
      (5 * NeptunesPride.gameConfig.developmentCostIndustry * (t.i + 1)) /
        ((r !== undefined ? r : t.r) / 100)
    );
  },
  calcUCS(t, r) {
    return Math.floor(
      (20 * NeptunesPride.gameConfig.developmentCostScience * (t.s + 1)) /
        ((r !== undefined ? r : t.r) / 100)
    );
  },
  calcUCG(t, r) {
    return 0 === NeptunesPride.gameConfig.buildGates
      ? 0
      : Math.floor(
          (100 * NeptunesPride.gameConfig.buildGates) /
            ((r !== undefined ? r : t.r) / 100)
        );
  },
  getMyStars() {
  ...
  comparePlayers() {
    ...
    Object.keys(NeptunesPride.universe.galaxy.players).forEach((key) => {
      ...
      if (player.uid !== NeptunesPride.universe.galaxy.player_uid) {
        ...
        const ships =
          me.total_strength - player.total_strength < 0
            ? `<span class="red-color">${
                me.total_strength - player.total_strength
              }</span>`
            : me.total_strength - player.total_strength;
        const cash =
          me.total_economy * 10 +
          me.tech.banking.level * 75 -
          (economyPerProdFromStars + bankingBonus);
        info.push({
          //    avatar: player.avatar,
          playerName: player.alias,
          cash: cash < 0 ? `<span class="red-color">${cash}</span>` : cash,
          economy:
            me.total_economy - player.total_economy < 0
              ? `<span class="red-color">${
                  me.total_economy - player.total_economy
                }</span>`
              : me.total_economy - player.total_economy,
          fleets:
            me.total_fleets - player.total_fleets < 0
              ? `<span class="red-color">${
                  me.total_fleets - player.total_fleets
                }</span>`
              : me.total_fleets - player.total_fleets,
          industry:
            me.total_industry - player.total_industry < 0
              ? `<span class="red-color">${
                  me.total_industry - player.total_industry
                }</span>`
              : me.total_industry - player.total_industry,
          science:
            me.total_science - player.total_science < 0
              ? `<span class="red-color">${
                  me.total_science - player.total_science
                }</span>`
              : me.total_science - player.total_science,
          stars:
            me.total_stars - player.total_stars < 0
              ? `<span class="red-color">${
                  me.total_stars - player.total_stars
                }</span>`
              : me.total_stars - player.total_stars,
          ships: ships < 0 ? `<span class="red-color">${ships}</span>` : ships,
          weapons:
            weapons < 0 ? `<span class="red-color">${weapons}</span>` : weapons,
          range:
            me.tech.propulsion.level - player.tech.propulsion.level < 0
              ? `<span class="red-color">${
                  me.tech.propulsion.level - player.tech.propulsion.level
                }</span>`
              : me.tech.propulsion.level - player.tech.propulsion.level,
          scanning:
            me.tech.scanning.level - player.tech.scanning.level < 0
              ? `<span class="red-color">${
                  me.tech.scanning.level - player.tech.scanning.level
                }</span>`
              : me.tech.scanning.level - player.tech.scanning.level,
          manufacturing:
            me.tech.manufacturing.level - player.tech.manufacturing.level < 0
              ? `<span class="red-color">${
                  me.tech.manufacturing.level - player.tech.manufacturing.level
                }</span>`
              : me.tech.manufacturing.level - player.tech.manufacturing.level,
        });
      ...
      tech.push({
        playerName: player.uid == me.uid ? `<span class="orange-color">${ player.alias}</span>` : player.alias,
        scanning: this.setColor(me.tech.scanning.level, player.tech.scanning.level, 'red', player.uid == me.uid ? 'orange' : '') ,
        manufacturing: this.setColor(me.tech.manufacturing.level, player.tech.manufacturing.level, 'red', player.uid == me.uid ? 'orange' : '') ,
        weapons: this.setColor(me.tech.weapons.level, player.tech.weapons.level, 'red', player.uid == me.uid ? 'orange' : '') ,
        research: this.setColor(me.tech.research.level, player.tech.research.level, 'red', player.uid == me.uid ? 'orange' : '') ,
        propulsion: this.setColor(me.tech.propulsion.level, player.tech.propulsion.level, 'red', player.uid == me.uid ? 'orange' : '') ,
        terraforming: this.setColor(me.tech.terraforming.level, player.tech.terraforming.level, 'red', player.uid == me.uid ? 'orange' : '') ,
        banking: this.setColor(me.tech.banking.level, player.tech.banking.level, 'red', player.uid == me.uid ? 'orange' : ''),
      });
    });
    ...
    if (counts.weapons && counts.ships) {
      recommendations.push(
        "At least one of our enemies has more ships and weapons. Need to increase production as our enemies are getting stronger than us."
      );
    }
    if (counts.weapons) {
      recommendations.push(
  ...
  setColor(compareValue, value, negativeColor, positiveColor){
    return compareValue < value ? `<span class="${negativeColor ? negativeColor : 'red'}-color">${value}</span>` :  `<span class="${positiveColor ? positiveColor : ''}-color">${value}</span>` ;
  },
  upgradeData() {
    const collection = [];
    ...
    Object.keys(NeptunesPride.universe.galaxy.stars).forEach((key) => {
      ...
      totalCostForEconomyAfterUpgrade += this.calcUCE(
        star,
        (player.tech.terraforming.level + 1) * 5 + star.nr
      );
      totalCostForIndustryAfterUpgrade += this.calcUCI(
        star,
        (player.tech.terraforming.level + 1) * 5 + star.nr
      );
      totalCostForScienceAfterUpgrade += this.calcUCS(
        star,
        (player.tech.terraforming.level + 1) * 5 + star.nr
      );
      totalCostForGateAfterUpgrade += this.calcUCG(
        star,
        (player.tech.terraforming.level + 1) * 5 + star.nr
      );
    ...
    return {
      economy: {
        ...
        saving: `${(
          (1 - totalCostForEconomyAfterUpgrade / totalCostForEconomy) *
          100
        ).toFixed(1)}%`,
      },
      industry: {
        ...
        saving: `${(
          (1 - totalCostForIndustryAfterUpgrade / totalCostForIndustry) *
          100
        ).toFixed(1)}%`,
      },
      science: {
        ...
        saving: `${(
          (1 - totalCostForScienceAfterUpgrade / totalCostForScience) *
          100
        ).toFixed(1)}%`,
      },
      gate: {
        ...
        saving: `${(
          (1 - totalCostForGateAfterUpgrade / totalCostForGate) *
          100
        ).toFixed(1)}%`,
      },
      manufactoring: {
        level: player.tech.manufacturing.level,
        currentShipsPerHour: Math.round(
          (player.total_industry * (player.tech.manufacturing.level + 5)) / 24
        ),
        upgradeShipsPerHour: Math.round(
          (player.total_industry * (player.tech.manufacturing.level + 1 + 5)) /
            24
        ),
      },
      banking: {
        ...
        currentPerProd:
          player.total_economy * 10 + player.tech.banking.level * 75,
        upgradePerProd:
          player.total_economy * 10 + (player.tech.banking.level + 1) * 75,
      },
    };
  },
  allPlayerStats() {
    ...
    Object.keys(NeptunesPride.universe.galaxy.players).forEach((key) => {
      ...
      collection.push({
        ...
        shipsPer30Mins: Math.round(
          (player.total_industry * (player.tech.manufacturing.level + 5)) / 24
        ),
        experimentationPoints: player.tech.research.level * 72,
        experimentationPointsTime: NeptunesPride.universe.timeToTick(
          player.tech.research.level * 72
        ),
      ...
      Object.keys(player.tech).forEach((k) => {
        tech[k] = {
          ...
          timeToNext: NeptunesPride.universe.timeToTick(
            (player.tech[k].level - (nextTechLevel - 1)) * 144 -
              (player.tech[k].research !== undefined
                ? player.tech[k].research
                : 0)
          ),
        };
      });
  ...
  starsInfo() {
    ...
    Object.keys(NeptunesPride.universe.galaxy.stars).forEach((key) => {
      ...
      if (star.puid >= 0) {
        ...
        info[player.uid] = info[player.uid]
          ? info[player.uid]
          : {
              player: player.alias,
              topStars: [],
        ...
        if (star.nr >= 40 || star.n >= 40)
          info[player.uid].topStars.push({
            name: star.n,
        ...
        if (star.puid !== NeptunesPride.universe.galaxy.player_uid) {
          Object.keys(NeptunesPride.universe.galaxy.stars).forEach((k) => {
            ...
            if (rangeStar.puid == NeptunesPride.universe.galaxy.player_uid) {
              if (
                this.isInRange(star, rangeStar, player.tech.propulsion.value)
              ) {
                info[player.uid].inRangeForAttack.push({
                  enemyStar: {
                    owner:
                      NeptunesPride.universe.galaxy.players[star.puid].alias,
                    starName: star.n,
                    NaturalResource: star.nr,
                    Terraformed: star.r,
                    ships:
                      this.getShipsOnOrbitingFleets(star).totalShips + star.st,
                    myStar: rangeStar.n,
                  },
                  myStar: {
                    owner: NeptunesPride.universe.galaxy.players[rangeStar.puid]
                      ? NeptunesPride.universe.galaxy.players[rangeStar.puid]
                          .alias
                      : "unknown",
                    ...
                    ships:
                      this.getShipsOnOrbitingFleets(rangeStar).totalShips +
                      rangeStar.st,
                  },
                });
              }
  ...
  checkTravelCapability(starId, toStarId) {
    ...
    const star =
      ...
      NeptunesPride.universe.galaxy.fleets[starId];
  ...
  updateRuler(starOrFleet, ruler) {
    ...
    if (t !== ruler.stars[ruler.stars.length - 1]) {
      "fleet" === t.kind && t.orbiting
        ? ruler.stars.push(t.orbiting)
        : ruler.stars.push(t);
  ...
  battle(starId, defending) {
    ...
    const defenderInfo = {
      starName: star.n,
      playerName: NeptunesPride.universe.galaxy.players[star.puid]
        ? NeptunesPride.universe.galaxy.players[star.puid].alias
        : "unknown",
      playerWeaponsLevel:
        NeptunesPride.universe.galaxy.players[star.puid] &&
        NeptunesPride.universe.galaxy.players[star.puid].tech.weapons.level,
      numberOfShips: star.st,
      shipsPer30Mins:
        NeptunesPride.universe.galaxy.players[star.puid] !== undefined &&
        Number(
          this.shipsPerTickByindustry(
            NeptunesPride.universe.galaxy.stars[starId],
            undefined,
            star.puid
          )
      ...
      defenderWeaponTech:
        NeptunesPride.universe.galaxy.players[star.puid] !== undefined &&
        NeptunesPride.universe.galaxy.players[star.puid].tech.weapons.level + 1,
    };
    if (defending) {
      ...
      Object.keys(NeptunesPride.universe.galaxy.fleets).forEach((key) => {
        ...
        fleet.o.forEach((waypoint) => {
          ...
          if (destinationStarId == star.uid && fleet.puid !== star.puid) {
            ...
            const minuteShips =
              Number(etaParts[1].replace("m", "")) >= 30 ? 1 : 0;
            const numberOfShipsOnArrival = shipsCount
              ? shipsCount +
                defenderInfo.shipsPer30Mins * 2 * hours +
                minuteShips
              : 0;
            const playerWeaponsLevel =
              NeptunesPride.universe.galaxy.players[fleet.puid] &&
              NeptunesPride.universe.galaxy.players[fleet.puid].tech.weapons
                .level;
            const battleInfo = this.fight(
              defenderInfo.defenderWeaponTech,
              playerWeaponsLevel,
              numberOfShipsOnArrival,
              fleet.st
            );
            ...
            if (!battleInfo.defendersShips) {
              ...
              while (!potentialDefendersShips) {
                potentialBattleInfo = this.fight(
                  defenderInfo.defenderWeaponTech,
                  shipsToWin,
                  playerWeaponsLevel,
                  fleet.st
                ...
                potentialDefendersShips = Math.floor(
                  potentialBattleInfo.defendersShips
                );
              }
            }
            ...
            attackers.push({
              ...
              minimumExtraShipsRequired: Math.floor(
                shipsToWin - numberOfShipsOnArrival
              ),
            });
          }
        });
  ...
  fight(
    defenderWeaponsTech,
    defendersShips,
    attackersWeaponsTech,
    attackersShips
  ) {
    for (var a = ""; !a; ) {
      if (((attackersShips -= defenderWeaponsTech), 0 >= attackersShips)) {
        a = "defender";
        break;
      }
      if (((defendersShips -= attackersWeaponsTech), 0 >= defendersShips)) {
        a = "attacker";
        break;
      }
    }
  ...
  getMyStars() {
    ...
    Object.keys(NeptunesPride.universe.galaxy.stars).forEach((key) => {
      const star = NeptunesPride.universe.galaxy.stars[key];
      if (star.puid === NeptunesPride.universe.galaxy.player_uid)
        collection[key] = NeptunesPride.universe.galaxy.stars[key];
    });
    return collection;
  },
  timeToTick(t) {
  ...
  calcShipsPerTick(starId, playerId, industryLevel) {
    ...
    var r =
        (industryLevel ? industryLevel : star.i) *
        (5 + player.tech.manufacturing.level),
      n = r / NeptunesPride.universe.galaxy.production_rate;
    return n !== Math.round(n) && (n = n.toFixed(2)), n;
  },
  alreadyTrackingFleets: {},
  calculatePotentialShipsByTime(time, star) {
    const shipsPerTick = this.calcShipsPerTick(
      star.uid,
      NeptunesPride.universe.galaxy.player_uid
    );
  ...
  closetStars(star) {
    ...
    Object.keys(myStarsList).forEach((key) => {
      ...
      if (
        travel.techLevel <=
        NeptunesPride.universe.galaxy.players[myStar.puid].tech.propulsion.level
      ) {
        collection.push({
          star: myStar,
          eta: travel.eta,
  ...
  checkPlayerNotifications() {
      ...
      if (previousData) {
        Object.keys(player.tech).forEach((t) => {
          if (player.tech[t].level > previousData[key].tech[t]) {
            notifications.push(
              `<span class="orange-color">${player.alias}</span> has increased <span class="orange-color">${t}</span> to <span class="orange-color">${player.tech[t].level}</span>`
            );
          }
        });
      }
    ...
    if (!previousData)
      localStorage.setItem("player-intel", JSON.stringify(currentData));
    ...
    Object.keys(NeptunesPride.universe.galaxy.fleets).forEach((key) => {
      ...
      if (enemeyFleet.o && enemeyFleet.o.length) {
        enemeyFleet.o.forEach((o) => {
          const myStarId = o[1];
          const enemyTravel = this.checkTravelCapability(
            enemeyFleet.uid,
            myStarId
          );
          ...
          if (
            ...
            this.alreadyTrackingFleets[enemeyFleet.uid].destination == myStarId
          )
            return;

          const potentialShips = this.calculatePotentialShipsByTime(
            enemyTravel.time,
            defendingStar
          );
          ...
          if (
            defendingStar &&
            defendingStar.puid == NeptunesPride.universe.galaxy.player_uid
          ) {
            this.talk(
              `Detected a fleet (${enemeyFleet.n}) coming t0 ${defendingStar.n} star`
            );
            ...
            const fleetOnRoute = this.areFleetsOnRouteForDefence(
              defendingStar,
              enemyTravel,
              battleStats[0].minimumExtraShipsRequired
            );
            if (fleetOnRoute) return;
            if (battleStats[0].minimumExtraShipsRequired) {
              this.talk(
                `Looks like we don't have enough ships to defend this star!`
              );
              ...
              Object.keys(myStarList).forEach((key) => {
                ...
                if (
                  travel.techLevel <=
                    NeptunesPride.universe.galaxy.players[myStar.puid].tech
                      .propulsion.level &&
                  travel.ly < enemyTravel.ly
                ) {
                  availableHelp.push({
                    star: myStar,
                    eta: travel.eta,
              ...
              if (availableHelp.length) {
                this.talk(
                  `We have a few stars that can help and get there in time!`
                );
                this.talk(
                  `Let me see if we have enough ships to send as backup`
                );
                let helpInfo = {
                ...
                availableHelp.forEach((closeStar) => {
                  if (closeStar.travel.ly < enemyTravel.ly) {
                    ...
                    const orbitingInfo = this.getShipsOnOrbitingFleets(
                      closeStar.star
                    );
                    //store star info for a group attack
                    helpInfo.totalShips +=
                      shipsOnClosetStar + orbitingInfo.totalShips;
                    helpInfo.star.push({
                      star: closeStar,
                      totalShips: shipsOnClosetStar + orbitingInfo.totalShips,
                    });
                    if (
                      totalStarShipsOnArrival +
                        shipsOnClosetStar +
                        orbitingInfo.totalShips >=
                      battleStats[0].minimumExtraShipsRequired
                    ) {
                      // I can send fleets to help
                      const shipsNeededFromFleet =
                        battleStats[0].minimumExtraShipsRequired -
                        shipsOnClosetStar;
                      //add ships to fleet
                      const fleetWithEnoughShips = orbitingInfo.fleets.find(
                        (f) => {
                      ...
                      if (fleetWithEnoughShips) {
                        this.talk(
                          `We have enough ships on ${closeStar.star.n} so I'll dispatch a fleet`
                        );
                        Api.transferShips(
                          closeStar.star,
                          fleetWithEnoughShips,
                          closeStar.star.st
                        );
                        Api.sendShip(fleetWithEnoughShips, defendingStar);
                        this.talk(
                          `Dispatched ${fleetWithEnoughShips.n} with ${
                            fleetWithEnoughShips.st
                          } and will take ${this.timeToTick(closeStar.eta)}`
                        );
                        shipsSent = true;
                      } else {
                        //TODO
                ...
                if (!shipsSent) {
                  this.talk(
                    "need to see if we can bring ships from more than one place or if it's worth defending"
                  );
                  if (availableHelp.length == 1) {
                    this.talk(
                      "we dont have any help need to consider saving fleets"
                    );
                    if (defendingStar.nr < 30 || defendingStar.r < 30) {
                      const orbitingFleets = this.getShipsOnOrbitingFleets(
                        defendingStar
                      );
                      if (orbitingFleets.fleets.length) {
                        Api.transferShips(
                          defendingStar,
                          orbitingFleets.fleets[0],
                          defendingStar.st
                        );
                    ...
                    availableHelp.forEach((closeStar) => {
                      ...
                      if (
                        sendingShips >= battleStats[0].minimumExtraShipsRequired
                      )
                        return;
                      if (closeStar.travel.ly < enemyTravel.ly) {
                        const orbitingFleets = this.getShipsOnOrbitingFleets(
                        ...
                        if (orbitingFleets.fleets.length) {
                          const shipsToTransfer =
                            closeStar.star.st - 10 ? closeStar.star.st - 10 : 0;
                          if (shipsToTransfer) {
                            Api.transferShips(
                              closeStar.star,
                              orbitingFleets.fleets[0],
